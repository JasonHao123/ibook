...
public class CopyOfMutableAclServiceImpl implements MutableAclService {
 ...
 */
     ...
     */
    protected IAclObjectIdentity retrieveObjectIdentityPrimaryKey(ObjectIdentity oid) {
        try {
    ...
    protected void createObjectIdentity(ObjectIdentity object, Sid owner) {
        IAclSid sid = createOrRetrieveSidPrimaryKey(owner, true);
        IAclClass clazz = createOrRetrieveClassPrimaryKey(object.getType(), true);
     ...
     */
    protected IAclSid createOrRetrieveSidPrimaryKey(Sid sid, boolean allowCreate) {
        ...
        List<IAclSid> sidIds = aclDao.findAclSidList(Boolean.valueOf(sidIsPrincipal), sidName);
     ...
     * @return the primary key or null if not found
     */
    protected IAclClass createOrRetrieveClassPrimaryKey(String type, boolean allowCreate) {
        List<IAclClass> classIds = aclDao.findAclClassList(type);
    ...
    public void deleteAcl(ObjectIdentity objectIdentity, boolean deleteChildren) throws ChildrenExistException {
        ...
        IAclObjectIdentity oidPrimaryKey = retrieveObjectIdentityPrimaryKey(objectIdentity);
    ...
    protected void createEntries(final MutableAcl acl) {
        ...
        List<IAclEntry> entries = new ArrayList<IAclEntry>();
        for(int i=0;i<acl.getEntries().size();i++) {
            ...
            IAclSid aclSid = aclDao.findAclSid(sid.getPrincipal());
            if(aclSid==null) {
    ...
    protected void updateObjectIdentity(MutableAcl acl) {
        IAclObjectIdentity parentId = null;

        if (acl.getParentAcl() != null) {
            Assert.isInstanceOf(ObjectIdentityImpl.class, acl.getParentAcl().getObjectIdentity(),
                "Implementation only supports ObjectIdentityImpl");

            IAclObjectIdentity oii = (IAclObjectIdentity) acl.getParentAcl().getObjectIdentity();
            parentId = retrieveObjectIdentityPrimaryKey(oii);
        }

        Assert.notNull(acl.getOwner(), "Owner is required in this implementation");

        IAclSid ownerSid = createOrRetrieveSidPrimaryKey(acl.getOwner(), true);
